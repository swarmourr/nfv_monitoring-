...
from api.deployer.connector import *
import requests
import urllib2
...
configs.read(os.getcwd()+"/config.conf")
...
def create_vm_agent(info={}):
    ...
    metric_exclude=[]
    mtr = info["metrics"]
    host = info["access_host"]
    for k, v in info["metrics"].iteritems():
        if v == True:
            metrics.append(k)
    id = ''.join(random.choice(chars) for x in range(8))
    name = "agent_vm_"+host["host ip"]
    monitor_type=info["agent"]["type"]

    # prepare template for agent
    copy("agent_api_template", "agent_temp/"+name)
    copy("operation/vm.py", "agent_temp/"+name+"/api/endpoint/")
    copy("operation/agent_vm.py", "agent_temp/"+name+"/api/impl/")
    copy("operation/models/vm_model.py", "agent_temp/"+name+"/api/models/")
    ...
    config.set('agent', 'metrics', metrics)
    config.set('agent', 'type', info["agent"]["type"])
    config.set('agent', 'activated', info["agent"]["activated"])
    config.add_section('server')
    config.set('server',"server_ip",host["server ip"])
    config.set('server',"server_port",host["server port"])
    config.add_section('DB')
    config.set('DB', 'alerts_url', info["database"]["alerts"]["flume_url"])
    config.set('DB', 'alerts_port', info["database"]["alerts"]["port"])
    config.set('DB', 'data_url', info["database"]["data"]["flume_url"])
    config.set('DB', 'data_port', info["database"]["data"]["port"])
    config.set('DB', 'hdfs_url', info["database"]["hdfs"]["hdfs_url"])
    config.set('DB', 'hdfs_port', info["database"]["hdfs"]["hdfs_port"])
    # add passive monitoring config
    if monitor_type.lower ()=="passive":
        config.add_section('contrainte')
        config.set('contrainte', 'mem', info["agent contrainte"]["mem"])
        #config.set('contrainte', 'disk', info["agent contrainte"]["disk"])
        config.set('contrainte', 'cpu', info["agent contrainte"]["cpu"])
        config.set('agent', 'refresh_period', info["agent"]["refresh_period"])
        copy("operation/passive_vm.py", "agent_temp/"+name+"/api/impl/")
        with open("agent_temp/"+name+"/api/impl/"+"passive_vm.py") as f:
            line = f.readlines()
            for k,v in info["agent contrainte"].iteritems():
                if float(v)==0:
            ...
            for i in range(len(metric_exclude)):
                ...
                for l in range(len(line)):
                    if find in line[l]:
                        line[l] = "#"+line[l]
            with open("agent_temp/"+name+"/api/impl/"+"passive_vm.py", 'w') as f:
                for item in line:
                    f.write(item)
        update_pass("ENABLE","agent_temp/"+name+"/app.py")


    with open("agent_temp/"+name+"/config.conf", 'w') as set:
        config.write(set)

    # write different methods to collect ressources informations
    update("update", "ENABLE", "agent_temp/"+name+"/api/endpoint/vm.py")
    ...
    for metric in metrics:
        if metric == "net":
            ...
            update("net", "ENABLE", "agent_temp/"+name+"/api/endpoint/vm.py")
            update("connections", "ENABLE", "agent_temp/"+name+"/api/endpoint/vm.py")

        elif metric == "disk":
            metric_code += inspect.getsource(getdiskUsage)
            metric_code += inspect.getsource(getwrrd)
            update("disk", "ENABLE", "agent_temp/"+name+"/api/endpoint/vm.py")

        elif metric == "cpu":
            metric_code += inspect.getsource(getcpuload)
            metric_code += inspect.getsource(getcpuloadcore)
            update("cpu", "ENABLE", "agent_temp/"+name+"/api/endpoint/vm.py")

        elif metric == "mem":
            metric_code += inspect.getsource(getRAM)
            update("mem", "ENABLE", "agent_temp/"+name+"/api/endpoint/vm.py")
        else:
            return "No metric defined"

    # type of monitoring :
    copy("agent_temp/"+name, "api/builder/"+name)

    # compresse agent
    compresse("api/builder/"+name)
    shutil.rmtree("api/builder/"+name)
    shutil.rmtree("agent_temp/"+name)
    copy("api/builder/"+name+".tar.gz", "agent_temp/")
    os.remove("api/builder/"+name+".tar.gz")

    # send agent via ssh
    k=0
    k = deploy("agent_temp/"+name+".tar.gz", host["host ip"], host["username"], info["agent"]["activated"])
    add_conf(info)
    #print   {"port" : k}
...
def create_cnt_agent(info={}):
    ...
    print metrics
    # id = ''.join(random.choice(chars) for x in range(8))
    name = "agent_cnt_"+host["host ip"]
    monitor_type=info["agent"]["type"]

    # prepare template for agent
    copy("agent_api_template", "agent_temp/"+name)
    copy("operation/cnt.py", "agent_temp/"+name+"/api/endpoint/")
    copy("operation/agent_cnt.py", "agent_temp/"+name+"/api/impl/")
    copy("operation/models/vm_model.py", "agent_temp/"+name+"/api/models/")
    ...
    if monitor_type.lower ()=="passive":
        ...
        config.set('contrainte', 'cpu', info["agent contrainte"]["cpu"])
        config.set('agent', 'refresh_period', info["agent"]["refresh_period"])
        copy("operation/passive_cnt.py", "agent_temp/"+name+"/api/impl/")
        with open("agent_temp/"+name+"/api/impl/passive_vm.py") as f:
            line = f.readlines()
            for k,v in info["agent contrainte"].iteritems():
                if float(v)==0:
    ...
    for metric in metrics:
        if metric == "net":
            ...
            metric_code += inspect.getsource(getdebit)
            metric_code += inspect.getsource(getconnections)
            update("net", "ENABLE", "agent_temp/"+name+"/api/endpoint/cnt.py")
            update("connections", "ENABLE", "agent_temp/"+name+"/api/endpoint/cnt.py")
        elif metric == "disk":
            metric_code += inspect.getsource(disk_cnt)
            metric_code += inspect.getsource(getwrrd_cnt)
            update("disk", "ENABLE", "agent_temp/"+name+"/api/endpoint/cnt.py")
            ...
            update("cpu", "ENABLE", "agent_temp/"+name+"/api/endpoint/cnt.py")

        elif metric == "mem":
            metric_code += inspect.getsource(mem_cnt)
            update("mem", "ENABLE", "agent_temp/"+name+"/api/endpoint/cnt.py")
        else:
            return "No metric defined"
    ...
    copy("api/builder/"+name+".tar.gz", "agent_temp/")
    os.remove("api/builder/"+name+".tar.gz")
    deploy("agent_temp/"+name+".tar.gz", host["host ip"], host["username"], info["agent"]["activated"])
    add_conf(info)
...
def create_jvm_agent(info={}):
    ...
    copy("agent_api_template", "agent_temp/"+name)
    copy("operation/jvm.py", "agent_temp/"+name+"/api/endpoint/")
    copy("operation/jvm_agents.py", "agent_temp/"+name+"/api/impl/")
    copy("operation/models/vm_model.py", "agent_temp/"+name+"/api/models/")
    ...
    if monitor_type.lower ()=="passive":
        ...
        config.set('contrainte', 'cpu', info["agent contrainte"]["cpu"])
        config.set('agent', 'refresh_period', info["agent"]["refresh_period"])
        copy("operation/passive_jvm.py", "agent_temp/"+name+"/api/impl/")
        with open("agent_temp/"+name+"/api/impl/"+"passive_jvm.py") as f:
            line = f.readlines()
            for k,v in info["agent contrainte"].iteritems():
                if float(v)==0:
                    metric_exclude.append(k)
            for i in range(len(metric_exclude)):
                ...
                for l in range(len(line)):
                    if find in line[l]:
                        line[l] = "#"+line[l]
            with open("agent_temp/"+name+"/api/impl/"+"passive_jvm.py", 'w') as f:
                for item in line:
                    f.write(item)
        update_pass("ENABLE","agent_temp/"+name+"/app.py")
    ...
    for metric in metrics:
        if metric.lower() == "connections":
            ...
            update("connections", "ENABLE", "agent_temp/"+name+"/api/endpoint/jvm.py")
        elif metric.lower() == "gc":
            metric_code += inspect.getsource(get_gc)
            update("gc", "ENABLE", "agent_temp/"+name+"/api/endpoint/jvm.py")
        elif metric.lower() == "threads":
            metric_code += inspect.getsource(get_threads)
            metric_code += inspect.getsource(cmd1)
            update("threads", "ENABLE", "agent_temp/"+name+"/api/endpoint/jvm.py")
        elif metric.lower() == "ressouces":
            metric_code += inspect.getsource(info_m)
            update("ressources", "ENABLE", "agent_temp/"+name+"/api/endpoint/jvm.py")
    """    try:
...
def update_agent_vnf(des):
    dict = {"enable": [], "disable": [],"contraint":{},"host":{}}
    for k, v in des["metrics"].iteritems():
        if v == True:
            dict["enable"].append(k)
        else:
            dict["disable"].append(k)
    dict["contraint"]=des["agent contrainte"]
    dict["host"]=des["access_host"]
    u = "http://"+des["access_host"]["host ip"]+":"+des["access_host"]["host port"]+"/api/vnf/update/"
...
def update_agent_anf(des):
    ...
    u = "http://"+des["access_host"]["host ip"]+":"+des["access_host"]["host port"]+"/api/jvm/update/"
    #r = requests.put("http://"+des["access_host"]["host ip"]+"/"+des["access_host"]["host port"]+"/api/vnf/update/", data=json.dumps(dict))
...
def delete_agents(info={}):
    ...
    delete_agent(info["host ip"],info["username"])


def delete_agent_db(info={}):
    print "je suis la "
    print '/monitoring/datas/type='+info["type"]+'/provider='+info["host ip"]
    tmp=configs.get("DB","hdfs").split(":")
    client = Client(tmp[0],int(tmp[1]))
...
def disable_agents(info={}):
    disable_agent(info["host ip"],info["username"])

def enable_agents(info={}):
    enable_agent(info["host ip"],info["username"])
