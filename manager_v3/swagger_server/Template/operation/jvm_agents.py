...
def info_m():
    ...
    for i in range(len(l)):
        ...
        n = subprocess.Popen('jstat -gc ' + pid + ' | awk \'{split($0,a,\" \"); print a[3]+a[4]+a[6]+a[8]}\' ', shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
def get_connections():
    ...
    for i in range(len(l)):
        cnx["Total"] = 0
        p1 = subprocess.Popen('lsof -i -a -p ' + str(l[i]).rstrip() + ' 2>/dev/null  | awk \'{print  $8,$9}\'|grep -v NAME', shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
def update_agent_anf(desc={}):
    print desc
    line = []
    with open(str(os.getcwd())+"/agent/api/builder/agent/api/endpoint/jvm.py") as f:
        line = f.readlines()
        ...
        for l in range(len(line)):
            if find in line[l]:
                if "#" in line[l]:
                    ...
                    line[l] = "# "+line[l]
    with open(str(os.getcwd())+"/agent/api/builder/agent/api/endpoint/jvm.py", 'w') as f:
        for item in line:
            f.write(str(item))
        return 204
    return 202
