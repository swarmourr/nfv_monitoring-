...
def info_cnt():
    infos = {}
    p = subprocess.Popen(('cat /proc/self/cgroup|grep memory:/docker/ |cut -d/ -f3'), shell=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
def update_agent_cnt(desc={}):
    line = []
    with open(str(os.getcwd())+"/agent/api/builder/agent/api/endpoint/cnt.py") as f:
        line = f.readlines()
    ...
    with open(str(os.getcwd())+"/agent/api/builder/agent/api/endpoint/cnt.py", 'w') as f:
        for item in line:
            f.write(str(item))
