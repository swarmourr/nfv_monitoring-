...
'''
def connect_ssh(ip, username,path,pwd):
    config.read(os.getcwd()+"/swagger_server/config.conf")
    ssh = SSHClient()
    ssh.load_system_host_keys()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.set_missing_host_key_policy(paramiko.WarningPolicy())
    if  pwd :   
        ssh.connect(ip, port=int(config.get("ssh","port")), username=username, password=pwd,timeout=30)
    else :  
...
def deploy(name, ip, username, state , pwd):
    if username == "root":
        path = "root"
    else:
        path = "home/"+username
    ssh = connect_ssh(ip, username, path , pwd)
...
def deploy_http(info={}):
    ...
    filename = os.getcwd()+"/swagger_server/Template/operation/fr.laas.anf.agent_1.0.0.201908052142.jar"
    url_agent= 'http://'+ info["_access_host"]["host ip"]+':'+info["_access_host"]["host port"]+'/system/console/bundles'
    url_config= 'http://'+  info["_access_host"]["host ip"]+':'+ info["_access_host"]["host port"]+'/services/config'
    ...
    requests.post(url_agent, files=files, data=data, auth=( info["_access_host"]["username"], info["_access_host"]["password"]))
    time.sleep(3)
...
def deploy_http_start(info={}):
    url_agent= 'http://'+ info["_host_ip"]+':'+info["_host_port"]+'/system/console/bundles/fr.laas.anf.agent'
    data = {
        'action': 'start',
        }
    requests.post(url_agent, data=data, auth=( info["_username"], info["_password"]))
...
def delete_agent(ip,username,pwd):
        if username == "root":
            path = "root"
        else:
            path = "home/"+username
...
def disable_agent(ip,username,pwd):
    if username == "root":
        path = "root"
    else:
        path = "home/"+username
...
def enable_agent(ip,username,pwd):
    if username == "root":
        path = "root"
    else:
        path = "home/"+username
